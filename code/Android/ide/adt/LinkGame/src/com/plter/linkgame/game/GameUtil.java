...
public class GameUtil {
	...
	public static RectF getRectFByCenter(PointF p) {
		return new RectF(p.x - Config.getCardWidth() / 2, p.y - Config.getCardHeight() / 2, p.x + Config.getCardWidth()
				/ 2, p.y + Config.getCardHeight() / 2);
	}
	...
	public static boolean testCards2(int i1, int j1, int i2, int j2, boolean genLines) {
		...
		PointF middle1Center = new PointF(GameUtil.getCenterXByIndexI(middleI1), GameUtil.getCenterYByIndexJ(middleJ1)), middle2Center = new PointF(
				GameUtil.getCenterXByIndexI(middleI2), GameUtil.getCenterYByIndexJ(middleJ2));

		if (currentGameCardsMap[middleI1][middleJ1] == null && testCards1(middleI1, middleJ1, i1, j1, false)
		...
		if (currentGameCardsMap[middleI2][middleJ2] == null && testCards1(middleI2, middleJ2, i1, j1, false)
		...
		for (int i = 0; i < currentGameCards.size(); i++) {
			...
			for (int j = i + 1; j < currentGameCards.size(); j++) {
				...
				if (card1.getPicture().getId() == card2.getPicture().getId()) {
					...
					if (testCards(currentLevel, currentGameCardsMap, card1.getIndexI(), card1.getIndexJ(),
							card2.getIndexI(), card2.getIndexJ(), false)) {
